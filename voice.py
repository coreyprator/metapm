"""
MetaPM Voice Capture API
Handles audio upload, Whisper transcription, Claude understanding
"""
...
from app.models.transaction import (
    VoiceCaptureRequest, VoiceCaptureResponse, TaskSummaryBrief
)
...
async def transcribe_audio(gcs_path: str) -> dict:
    ...
    async with httpx.AsyncClient() as http_client:
        ...
        return {
            ...
            # based on whether it detected the language correctly
...
async def text_capture(
    ...
    """
    Process text input (same flow as voice, minus transcription).
    Useful for web/API input.
    """
    start_time = time.time()
    
    # Understand with Claude
    claude_result = await understand_with_claude(text, project_code)
    ...
    return {
        "conversationGuid": str(conv_guid),
        "transactionGuid": str(trans_result[0]["TransactionGUID"]),
        "aiResponse": claude_result.get("response"),
        "extractedIntent": claude_result.get("intent"),
        "extractedProjectCode": claude_result.get("project_code"),
        "taskCreated": task_created,
        "processingTimeMs": total_time
