...
### ⚠️ Critical: Database User
**DB_USER = `sqlserver`** — MetaPM uses the shared sqlserver login, NOT a dedicated `metapm_user`.
This is unique among the portfolio apps. All other projects use dedicated DB users.
...
### Dashboard
`static/dashboard.html` (185KB) is the primary UI entry point. Contains status bar filtering,
task management, and project overview. This file is the heart of MetaPM's user interface and
should be inventoried in any frontend audit.
...
### ~~Google Calendar~~ — **DEPRECATED**

~~Google Calendar Integration~~ — **DEPRECATED** per project lead decision 2026-02-15.
Minimal value relative to auth friction and maintenance cost.
...
8. **`app/core/security.py` referenced but missing**: README references this file but it does not exist in the codebase. Either dead documentation or planned-but-never-built. Remove reference or create file.

9. **Two overlapping project tables**: `Projects` (schema.sql) and `roadmap_projects` (migration 8) serve similar purposes. Architectural debt — consolidate in future MetaPM roadmap work.

10. **No CI/CD pipeline**: No `.github/` directory. Every deploy is manual via `gcloud run deploy`. Only project in portfolio without GitHub Actions. P1 to add.
